...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (typeof website === 'string' && website.trim() !== '') {
      console.warn('[antispam] honeypot triggered (send-booking)');
    ...
    if (typeof formRenderedAt === 'number' && Date.now() - formRenderedAt < 3000) {
      console.warn('[antispam] submitted too fast (send-booking)');
      return NextResponse.json({ success: true }, { status: 200 });
    }
    ...
    if (!checkRateLimit(clientIp)) {
      console.warn('[antispam] rate limit exceeded (send-booking)', clientIp);
      return NextResponse.json(
